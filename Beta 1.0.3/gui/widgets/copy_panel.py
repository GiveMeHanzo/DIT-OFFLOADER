...
class CopyPanel(QWidget):
    ...
    def set_running(self, running: bool) -> None:
        for w in (self.add_files_btn, self.add_dir_btn, self.sel_all_btn,
                  self.rm_src_btn, self.dst_edit, self.job_name_edit,
                  self.policy_combo, self.report_cb, self.rename_cb,
                  self.sscl_import_btn, self.sscl_edit):
            w.setEnabled(not running)
        self.start_btn.setEnabled(not running)
        self.verify_btn.setEnabled(not running)
        self.start_btn.setText("⏸  运行中…" if running else "▶  开始拷贝")

...
from config import REPORT_SUFFIX, XML_LOG_SUFFIX, is_video
...
def generate_html_report(
    ...
    # 报告文件名跟随 XML 日志名（单一事实来源）：同名追加拷贝时日志为
    # JobName-1_log.xml，报告相应为 JobName-1_report.html，不覆盖上一次报告。
    xml_name = os.path.basename(getattr(summary, "xml_path", "") or "")
    if xml_name.endswith(XML_LOG_SUFFIX):
        report_base = xml_name[: -len(XML_LOG_SUFFIX)]
    else:
        report_base = summary.job_name
    html_path = os.path.join(
        summary.dest_root,
        f"{report_base}{REPORT_SUFFIX}",
    )
...
def _process_videos(
    ...
    def handle(rec: dict) -> dict:
        try:
            ...
            frames = FrameSet()
            if can_probe:
                # 优先 probe/抽帧目标盘上的拷贝产物：内容与源一致（已校验），
                # 且作业结束后源卡常被拔出——旧版读源卡会导致报告整行 Error，
                # 还会在用户开下一单时让最多 12 个 ffmpeg 进程锤同一张卡。
                probe_path = rec["dest"] if os.path.isfile(rec.get("dest") or "") \
                    else rec["src"]
                meta = probe_video(probe_path)
                dur = meta.duration if meta else 0.0
                frames = extract_frames(probe_path, duration=dur)
